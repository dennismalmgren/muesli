...
def retrace_advantage_estimate(
    gamma: float,
    log_pi: torch.Tensor,
    log_mu: torch.Tensor,
    ...
    """Retrace implementation using a state-value function for next states.

    Args:
        gamma (float): Discount factor.
        log_pi (Tensor): Log probabilities under the target policy for chosen actions.
                         Shape: [..., TimeSteps, *F]
        log_mu (Tensor): Log probabilities under the behavior policy for chosen actions.
                         Same shape as log_pi.
        state_action_value (Tensor): Q(s,a) values for the chosen actions.
                                     Shape: [..., TimeSteps, *F]
        next_state_value (Tensor): V(s') values for the next states.
                                    Shape: [..., TimeSteps, *F]
        reward (Tensor): Observed rewards for the chosen actions.
                         Shape: [..., TimeSteps, *F]
        done (Tensor): Boolean flags indicating end of the episode.
                       Shape: [..., TimeSteps, *F]
        terminated (Tensor, optional): Boolean flags for terminal states. If None, done is used.
        rho_thresh (float or Tensor, optional): Truncation parameter for importance weights (c = min(1, rho)).
        time_dim (int): Dimension of the time steps.

    Returns:
        Tuple[torch.Tensor, torch.Tensor]:
            advantages (Tensor): Retrace advantage estimates, \( Q^{ret}(s,a) - Q(s,a) \).
            q_retrace (Tensor): Retrace-corrected Q-values \( Q^{ret}(s,a) \).

    Note:
        This implementation uses a state-value function \( V(s') \) instead of sampling
        from \( Q(s', a') \) to compute the expected next state value.
    """

    if not (next_state_value.shape == state_action_value.shape == reward.shape == done.shape):
        raise RuntimeError("Shape mismatch among input tensors (Q-values, rewards, done).")

    device = state_action_value.device
    if not isinstance(rho_thresh, torch.Tensor):
        rho_thresh = torch.tensor(rho_thresh, device=device)
    rho_thresh = rho_thresh.to(device)

    # Handle termination and discounting
    not_done = (~done).int()
    if terminated is None:
        not_terminated = not_done
    else:
        not_terminated = (~terminated).int()

    discounts = gamma * not_terminated

    # Compute importance weights
    rho = (log_pi - log_mu).exp()
    clipped_rho = torch.clamp(rho, max=rho_thresh)

    # Initialize the Retrace offset (Q^ret - Q)
    q_retrace_offsets = [torch.zeros_like(state_action_value[..., -1, :])]

    # Backward pass to compute Retrace offset
    for i in reversed(range(state_action_value.shape[time_dim])):
        discount_t = discounts[..., i, :]
        delta_t = clipped_rho[..., i, :] * (
            reward[..., i, :] + discount_t * next_state_value[..., i, :] - state_action_value[..., i, :]
        ) + discount_t * clipped_rho[..., i, :] * q_retrace_offsets[-1]
        q_retrace_offsets.append(delta_t)

    # Reverse and stack offsets
    q_retrace_offsets = torch.stack(q_retrace_offsets[1:], dim=time_dim)
    q_retrace_offsets = torch.flip(q_retrace_offsets, dims=[time_dim])

    # Q^ret(s,a) = Q(s,a) + q_retrace_offset
    q_retrace = state_action_value + q_retrace_offsets

    # Advantages = Q^ret(s,a) - Q(s,a)
    advantages = q_retrace_offsets

    return advantages, q_retrace
...
class ReTrace(ValueEstimatorBase):
    def __init__(
        self,
        *,
        ...
        reward_network: TensorDictModule,                
    ...
    def forward(
        ...
        *,
        ...
        """Computes the ReTrace correction given the data in tensordict.
        ...
        with hold_out_net(self.reward_network):
            data_in = tensordict.select(*["observation", "action"])
            data_out = self.reward_network(data_in)
            reward_value = data_out.get("reward_value")
            state_action_value = reward_value + self.gamma * next_value
        ...
        adv, value_target = retrace_advantage_estimate(
            gamma,
            log_pi,
            log_mu,
            state_action_value,
...
class ReTraceV(ValueEstimatorBase):
