...
def resettable_cumsum(log_values, is_init, e_I):
    """
    Perform a cumulative scan with resets using cumsum.

    log_values: Tensor of log-transformed states (shape: [batch, time, hidden_dim]).
    is_init: Tensor of reset flags (shape: [batch, time]).
    e_I: Identity element for the monoid (log-space scalar or tensor).
    """
    # Ensure is_init has the same shape as log_values along time dimension
    #is_init = is_init.unsqueeze(-1)  # Shape: [batch, time, 1]

    # Compute the cumulative reset mask
    reset_mask = torch.cumsum(is_init, dim=1)  # Shape: [batch, time, 1]

    # Reset cumulative sum at boundaries
    # Accumulate log_values and reset at each new episode boundary
    cumulative_log_values = torch.cumsum(log_values * (~is_init), dim=1) + e_I * is_init

    # Only keep the current accumulated value, masking invalid accumulations
    reset_log_values = cumulative_log_values - cumulative_log_values * reset_mask

    return reset_log_values

def resettable_logcumsumexp(log_values, is_init, e_I):
    """
    Perform a cumulative log-sum-exp scan with resets.

    log_values: Tensor of log-transformed states (shape: [batch, time, hidden_dim]).
    is_init: Tensor of reset flags (shape: [batch, time]).
    e_I: Log-space identity element for the monoid.
    """
    # Ensure is_init has the same shape as log_values along time dimension
    is_init = is_init.unsqueeze(-1)  # Shape: [batch, time, 1]

    # Convert reset flag to a cumulative mask
    reset_mask = torch.cumsum(is_init, dim=1)  # Shape: [batch, time, 1]

    # Compute cumulative logsumexp with resets
    shifted_log_values = log_values + torch.log(1 - is_init + 1e-10)  # Mask resets
    cumulative_log_values = torch.logcumsumexp(shifted_log_values, dim=1)

    # Apply resets
    reset_log_values = torch.where(
        reset_mask > 0,
        e_I,  # Reset to identity where mask is active
        cumulative_log_values
    )

    return reset_log_values

# def heinsen_associative_scan_log2(log_z, log_values):
...
# def resettable_heinsen_associative_scan_log(log_coeffs, log_values, is_init, e_I):
#     a_star = log_coeffs.cumsum(dim = 1)
#     a_star_2 = resettable_cumsum(log_coeffs, is_init, e_I)
#     log_h0_plus_b_star = (log_values - a_star).logcumsumexp(dim = 1)
#     log_h = a_star + log_h0_plus_b_star
#     return log_h.exp()
