...
class QOrdinalClipPPOLoss(PPOLoss):
    ...
    def _two_hot(self, target_value, support: torch.Tensor):
        target_value = target_value.clamp(support.min(), support.max())
        lower_idx = torch.searchsorted(support, target_value.squeeze(), right=False) - 1
        upper_idx = lower_idx + 1
        
        lower_support = support[lower_idx]
        upper_support = support[upper_idx]
