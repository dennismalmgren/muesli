...
from kalmanfilter import AdaptiveKalmanFilterNN, AdaptiveKalmanFilterNN2
from transforms import KalmanReward
# ====================================================================
# Environment utils
# --------------------------------------------------------------------
...
def make_env(env_name="HalfCheetah-v4", device="cpu", from_pixels: bool = False):
    ...
    env.append_transform(KalmanReward())
...
def make_ppo_models_state(proof_environment, device):
    ...
    prediction_filter = AdaptiveKalmanFilterNN2(
        observation_features = input_shape[-1],
        action_features = num_outputs #predict next state, value by value
    )

    kalman_module = TensorDictModule(
        module = prediction_filter,
        in_keys=["state_prediction", "previous_action", "action", "observation", "is_init"],
        out_keys=[("next", "state_prediction"), ("next", "previous_action"), ("next", "prediction_error")]
    )
    ...
    actor_module = TensorDictSequential(policy_module, kalman_module)
    ...
    return actor_module, value_module, policy_module, kalman_module


def make_ppo_models(env_name, device):
    proof_environment = make_env(env_name, device=device)
    actor, critic, policy, kalman = make_ppo_models_state(proof_environment, device=device)
    return actor, critic, policy, kalman
