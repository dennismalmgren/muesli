...
class EnsembleModule(TensorDictModuleBase):
    ...
    def forward(self, tensordict: TensorDict) -> TensorDict:
        input = tensordict.select(*self.in_keys)
        result = self.vmapped_forward(input, self.params_td)
        batch_dims = range(1, len(tensordict.shape) + 2)
        for key in self.out_keys:
            tensordict[key] = result[key].permute(*batch_dims, 0)
        return tensordict #self.vmapped_forward(tensordict, self.params_td)
