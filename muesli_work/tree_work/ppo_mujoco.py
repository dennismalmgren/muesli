...
def main(cfg: "DictConfig"):  # noqa: F821
    ...
    collector = SyncDataCollector(
        create_env_fn=make_env(cfg.env.env_name, device, rnn),
        policy=actor,
    ...
    # Create data buffer
    #sampler = SliceSamplerWithoutReplacement(num_slices=1)
    ...
    for i in range(total_iter):
        ...
        with timeit("collecting"), torch.no_grad():
            ...
            for j in range(128):
                actor(td_)
                td, td_ = train_env.step_and_maybe_reset(td_)
                tds.append(td)
            data = torch.stack(tds, dim=0)
