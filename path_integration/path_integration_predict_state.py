...
def main(cfg: "DictConfig"):  # noqa: F821
    ...
    slice_len = 3 #use first two to predict the third
    slice_count_in_batch = 64
...
#    batch_size = num_slices * trajectory_length
    ...
    # trajs = dat.reshape((num_slices, -1))
    # min_per_dim = torch.min(torch.min(trajs['observation'], dim = 0)[0], dim=0)[0]
    # max_per_dim = torch.max(torch.max(trajs['observation'], dim = 0)[0], dim=0)[0]
    # State + Vel + Rot
    ...
    # State
    #model = MLP(in_features = 8, out_features = 8, num_cells = [64, 64])
