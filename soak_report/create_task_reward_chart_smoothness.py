...
run_path_ppo_smoothing = "dennisathome/opus_smoothing/07zl5slc"
#run = api.run(f"{entity}/{project}/{run_id}")
run = api.run(run_path_ppo_smoothing)
metric_id = "train/OpusSmoothingReward_smooth_pdot"
history = run.history()
...
metric_id = "train/OpusSmoothingReward_smooth_p"
metric = []
steps = []
last_step_id = -1
history = run.scan_history(keys=["trainer/step", metric_id])
...
plt.title('Smoothness reward (average)')
plt.legend()
